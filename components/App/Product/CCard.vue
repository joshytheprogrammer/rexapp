<template>
  <li class="flex flex-col justify-center md:grid grid-cols-5 gap-4 justify-between items-center border-b py-4">
    ...
    <span class="col-span-1 block py-2 text-gray-700 font-semibold text-sm text-center"> {{ formatPrice(data.product.price.min) }} <br> - <br> {{ formatPrice(data.product.price.max) }}</span>
